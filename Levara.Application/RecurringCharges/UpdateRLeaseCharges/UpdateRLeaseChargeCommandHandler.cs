...
public class UpdateRLeaseChargeCommandHandler : ICommandHandler<UpdateRLeaseChargeCommand, UpdateRLeaseChargeCommandResponse>
{
    ...
    public UpdateRLeaseChargeCommandHandler(IUnitOfWork unitOfWork,
    ...
    public async Task<OperationResult<UpdateRLeaseChargeCommandResponse>> Handle(UpdateRLeaseChargeCommand command)
    {
        // 1. Obtener el RecurringCharge existente con Lease y historial de instancias
        ...
        if (existingCharge == null)
            return OperationResult<UpdateRLeaseChargeCommandResponse>.ErrorResult(
                new ErrorDetails(404, $"RecurringCharge with id {command.Id} not found"));

        // 2. Validar ownership (si se proporciona OwnerId)
        if (command.OwnerId.HasValue && existingCharge.Lease?.OwnerId != command.OwnerId.Value)
            return OperationResult<UpdateRLeaseChargeCommandResponse>.ErrorResult(
                new ErrorDetails(403, "Access denied to this RecurringCharge"));

        // 3. Validar LeaseChargeType
        if (!await _leaseChargeTypeRepository.AnyAsync(lct => lct.Id == command.LeaseChargeTypeId))
            return OperationResult<UpdateRLeaseChargeCommandResponse>.ErrorResult(
                new ErrorDetails(404, $"LeaseChargeType with id {command.LeaseChargeTypeId} not found"));
        ...
        var response = new UpdateRLeaseChargeCommandResponse
        {
            Id = existingCharge.Id,
        };

        return OperationResult<UpdateRLeaseChargeCommandResponse>.SuccessResult(response);
    }

    private OperationResult<UpdateRLeaseChargeCommandResponse> ValidateBusinessRules(
        RecurringCharge existingCharge, UpdateRLeaseChargeCommand command, IEnumerable<RecurringChargeInstance> processedInstances)
    {
        ...
        if (command.IsRecurrent == true)
        {
            ...
            // Validar que las fechas estén dentro del rango del Lease
            if (command.StartDate < existingCharge.Lease!.DateFrom)
                return OperationResult<UpdateRLeaseChargeCommandResponse>.ErrorResult(
                    new ErrorDetails(400, $"Recurring charge StartDate ({command.StartDate:yyyy-MM-dd}) cannot be before lease StartDate ({existingCharge.Lease.DateFrom:yyyy-MM-dd})"));

            if (command.EndDate > existingCharge.Lease!.DateTo)
                return OperationResult<UpdateRLeaseChargeCommandResponse>.ErrorResult(
                    new ErrorDetails(400, $"Recurring charge EndDate ({command.EndDate:yyyy-MM-dd}) cannot be after lease EndDate ({existingCharge.Lease.DateTo:yyyy-MM-dd})"));
        ...
        return OperationResult<UpdateRLeaseChargeCommandResponse>.SuccessResult(null);
    }

    private bool DetermineIfNeedsRecalculation(RecurringCharge existingCharge,
        UpdateRLeaseChargeCommand command)
    {
        ...
        UpdateRLeaseChargeCommand command)
    {
        ...
        existingCharge.LeaseChargeTypeId = command.LeaseChargeTypeId!.Value;
