...
public class DeleteRMaintenanceChargeCommandHandler : ICommandHandler<DeleteRMaintenanceChargeCommand, DeleteRMaintenanceChargeCommandResponse>
{
    ...
    public DeleteRMaintenanceChargeCommandHandler(IUnitOfWork unitOfWork,
    ...
    public async Task<OperationResult<DeleteRMaintenanceChargeCommandResponse>> Handle(DeleteRMaintenanceChargeCommand command)
    {
        var recurringCharge = await _recurringChargeRepository.GetByIdAsync(command.Id!.Value);
        if (recurringCharge == null)
            return OperationResult<DeleteRMaintenanceChargeCommandResponse>.ErrorResult(
        ...
        return OperationResult<DeleteRMaintenanceChargeCommandResponse>.SuccessResult(new());
    }
}
