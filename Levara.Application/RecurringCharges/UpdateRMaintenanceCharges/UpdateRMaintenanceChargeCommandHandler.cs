...
public class UpdateRMaintenanceChargeCommandHandler : ICommandHandler<UpdateRMaintenanceChargeCommand, UpdateRMaintenanceChargeCommandResponse>
{
    ...
    public UpdateRMaintenanceChargeCommandHandler(IUnitOfWork unitOfWork,
    ...
    public async Task<OperationResult<UpdateRMaintenanceChargeCommandResponse>> Handle(UpdateRMaintenanceChargeCommand command)
    {
        ...
        if (existingCharge == null)
            return OperationResult<UpdateRMaintenanceChargeCommandResponse>.ErrorResult(
        ...
        // 3. Validar MaintenanceType
        if (!await _maintenanceTypeRepository.AnyAsync(lct => lct.Id == command.MaintenanceTypeId))
            return OperationResult<UpdateRMaintenanceChargeCommandResponse>.ErrorResult(
                new ErrorDetails(404, $"MaintenanceType with id {command.MaintenanceTypeId} not found"));
        ...
        var response = new UpdateRMaintenanceChargeCommandResponse
        {
            Id = existingCharge.Id,
        };

        return OperationResult<UpdateRMaintenanceChargeCommandResponse>.SuccessResult(response);
    }

    private OperationResult<UpdateRMaintenanceChargeCommandResponse> ValidateBusinessRules(
        RecurringCharge existingCharge, UpdateRMaintenanceChargeCommand command, IEnumerable<RecurringChargeInstance> processedInstances)
    {
        ...
        return OperationResult<UpdateRMaintenanceChargeCommandResponse>.SuccessResult(new());
    }

    private bool DetermineIfNeedsRecalculation(RecurringCharge existingCharge,
        UpdateRMaintenanceChargeCommand command)
    {
